...
static int parseLogIdNum(const std::string& logId) {
    ...
    std::string digits = logId.substr(1);        // drop leading 'L'
    int value = 0;
...
void markAttendance(const std::string& courseCode, const std::string& date) {
    ...
    // 2. Snapshot the current file into the backup BEFORE writing anything
    g_attendanceBackup = readTXT("attendance_log.txt");
    g_backupExists     = true;

    // 3. Determine starting log_id for this batch
    //    (We read from backup since it reflects the file before this session)
...
double getAttendancePct(const std::string& roll, const std::string& courseCode) {
    ...
    for (int i = 0; i < (int)allRows.size(); i++) {
        ...
        // "A" contributes nothing — intentionally not counted
    }

    if (totalSessions == 0) return 0.0;

    // Formula from spec: (present + 0.5 * late) / totalSessions * 100
...
void printDailySheet(const std::string& courseCode, const std::string& date) {
    ...
    // Filter rows that match the given course and date
